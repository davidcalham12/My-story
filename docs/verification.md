...
Status, 2026-09-22: **83 backend tests and 9 frontend tests, on the mock engine,
...
**Why accepted:** temporarily, and reluctantly. The mock engine can drive a
chapter to three failures, so this gap is closable — nothing but work stands in
the way. It is written here precisely so it does not pass for done.
...
**Reviewed by:** the next spec that touches `chapters/`. It closes then, or this
row gets a date and stays.
...
| `SKILL.md` ↔ `flow.yaml` | the procedure and the contract diverging in silence |
