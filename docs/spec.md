...
| 2026-09-24 | **The buyer chooses the length: the full novel (10 chapters, profile `exam`, the default) or a short sample (1 chapter, profile `eval`).** Supersedes SPEC-EXAM-002's "shown, never editable" length field. The chapter count still comes from the profile, never from the form | the owner could not try the product without paying for ten chapters; the one-chapter profile already existed for the evals | the owner, in chat to the coordinating session: "no me deja cambiar la longitud", asking for the length to be changeable |
