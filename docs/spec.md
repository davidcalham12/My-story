...
| 2026-09-24 | **The example novel's run (02412b7fe29e) may spend up to 75 USD in total to finish chapters 9 and 10** — names the protected budget ceiling (AGENTS.md §6) for this one resumption only; the `exam` profile stays at 60. The run had stopped itself at 52.83 of 60 with 8 of 10 chapters promoted and published as v1; the resumption writes chapters 9-10 through the same gate (threshold 8, three attempts, patch-then-halt, all unchanged) and publishes the complete book as the next version, v1 staying readable | the exam asks for a ten-chapter novel; the measured cost of a chapter (4-6 USD) put the remainder at 10-15 USD above the 60 ceiling | the owner, in chat to the coordinating session: "y la opcion b", choosing B ("aprobar por escrito un techo nuevo para la reanudación") after being told it would cost about 10-15 USD more, exceed the 60 USD ceiling, and that the figure proposed was 75 USD |
| 2026-09-24 | **The orchestrator runs on Sonnet in the `exam` and `eval` profiles** (supersedes "the orchestrator stays on the session model"). Not Haiku: on Haiku it stopped dispatching the project's agents. The run resuming chapters 9-10 was launched before this and keeps its model | the example novel took 119 minutes for 8 chapters and one orchestrator turn on the session model cost about 1 USD; the orchestrator's turns were most of the bill | the owner, in chat to the coordinating session: "baja el orquestador y menos criticos tambien me gustaria que los gastos bajen si se puede" |
| 2026-09-24 | **A reader change is accepted only if the changed fact arrives**: each affected chapter's writer packet carries the new text as mandatory, and before a chapter counts as accepted an arithmetic check confirms the new text is present and the old one (with its plural and case variants) is absent; a failed check is a failed attempt with that reason. It is the reader change's acceptance condition, like the `chatter` rule — **not a seventh characteristic**, and the threshold is unchanged | v3's ch03 was promoted with 0 mentions of the observatory: the gate passed because no critic checks that a changed fact is present, and "a change that does not arrive is not a change" | "Rehacer el cap. 3 con control", answering the build session's proposal (coordinating session, ~21:15 UTC) |
| 2026-09-24 | **A per-novel spending cap**: Continue asks for no figure (SPEC-007 §8), but is refused once a novel's known spend reaches `budget.novel_ceiling_multiple` (2) × its profile's ceiling | SR-05: with a fresh ceiling per continuation, a novel's total was bounded only by how often Continue is pressed | "Tope automático por novela", answering the build session's question in its own session |
