...
def test_a_stream_with_no_agent_result_writes_no_call_row(client, db):
    """The fixture was recorded by a CLI whose Agent results carried no usage.
    `task_progress` alone gives one blended total and no output, and no longer
    makes a row: absent, not a guessed split (agent_usage, 2026-09-24)."""
    run_id = client.post("/api/runs", json={"premise": PREMISE}).json()["id"]
    _wait(client, run_id)
    assert db.execute("SELECT COUNT(*) FROM calls WHERE run_id = ?", (run_id,)).fetchone()[0] == 0
